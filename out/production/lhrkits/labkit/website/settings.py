...
class Settings:
    ...
    def __init__(self, default_config):
        ...
        self.config['BLOG_TITLE'] = ''
        self.config['BLOG_DESCRIPTION'] = ''
        self.config['MOLECULE_LIST_DESCRIPTION'] = ''
        self.config['DATA_LICENCE'] = ''
    ...
    def get_config(self):
        try:
            ...
            if cursor:
                ...
                # print self.config['BLOG_TITLE']
                self.config['BLOG_DESCRIPTION'] = cursor.get(
                    'description', self.config['BLOG_DESCRIPTION'])
                self.config['MOLECULE_LIST_DESCRIPTION'] = cursor.get(
                    'molecule_list_description', self.config['MOLECULE_LIST_DESCRIPTION'])
                self.config['DATA_LICENCE'] = cursor.get(
                    'data_licence', self.config['DATA_LICENCE'])
            return self.config
        except Exception, e:
    ...
    def is_installed(self):
        ...
        applications_cnt = self.config['APPLICATIONS_COLLECTION'].find().count()
        if posts_cnt and users_cnt and configs_cnt and applications_cnt :
    ...
    def install(self, blog_data, user_data):
        ...
        postClass = molecule.Post(self.config)
        applicationClass = application.Application(self.config)
        ...
        try:
            ...
            self.config['APPLICATIONS_COLLECTION'].ensure_index([('date', 1)])
            ...
            application_data = {'title': 'Hello World!',
                         'preview': 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod',
                         'body': 'tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam',
                         'tags': [],
                         'email': 'airhenry@gmail.com',
                         'author': user_data['_id']}
            application = applicationClass.validate_application_data(application_data)

            user_create = userClass.save_user(user_data)
            post_create = postClass.create_new_post(molecule)
            application_create = applicationClass.create_new_application(application)
